...
import { 
  ...
  DialogContent, DialogContentText, DialogActions, Button, Checkbox, Paper, Avatar
} from '@mui/material';
...
const RecyclingBinPage: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
    ...
    const handleSelectDrawing = (id: number) => {
        setSelectedDrawings(prev => prev.includes(id) ? prev.filter(selectedId => selectedId !== id) : [...prev, id]);
    };
    ...
    return (
        <Box sx={{ padding: 4, minHeight: '100vh', marginLeft: { xs: 0, md: '100px' } }}>
            ...
            {status === 'loading' ? (
                ...
                <Paper elevation={2}>
                    <List>
                        {deletedPaintedDrawings.map(drawing => (
                            <ListItem key={drawing.id} secondaryAction={
                                <Box>
                                ...
                                <ListItemIcon>
                                    {/* Replace the generic icon with the actual image thumbnail */}
                                    <Avatar 
                                        variant="rounded" 
                                ...
                                <ListItemText
                                    ...
                                    // secondary={`נמחק ${formatDistance(new Date(drawing.deletedAt), new Date(), { locale: he })}`}
                                    sx={{ marginLeft: 2 }}
                                />
                            </ListItem>
                        ))}
